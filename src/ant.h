...
void move_forward(int ant_i) {    // Ant index
    switch (ANTS_ARRAY[ant_i].rotation) {
        case UP:
            if (ANTS_ARRAY[ant_i].yp > 0)
                ANTS_ARRAY[ant_i].yp--;
            else
                ANTS_ARRAY[ant_i].yp = WINDOW_H / CELL_SIZE - 1;
            break;
        case DOWN:
            if (ANTS_ARRAY[ant_i].yp < WINDOW_H / CELL_SIZE - 1)
                ANTS_ARRAY[ant_i].yp++;
            else
                ANTS_ARRAY[ant_i].yp = 0;
            break;
        case LEFT:
            if (ANTS_ARRAY[ant_i].xp > 0)
                ANTS_ARRAY[ant_i].xp--;
            else
                ANTS_ARRAY[ant_i].xp = WINDOW_W / CELL_SIZE - 1;
            break;
        case RIGHT:
            if (ANTS_ARRAY[ant_i].xp < WINDOW_W / CELL_SIZE - 1)
                ANTS_ARRAY[ant_i].xp++;
            else
                ANTS_ARRAY[ant_i].xp = 0;
            break;
        default:
            break;
    }
...
int rotate(int rotation, int facing_direction) {
    ...
    fprintf(stderr, "Error getting the rotation.\n");
    return -1;
}
...
/*
 * In order to access the 2d array, we pass the x and y size and then refer to the
 * array as cell_array[desiredx * x_size + desiredy] = ... ; Credit to:
...
int move_ant(int* cell_array, int x_size, int ant_i) {
    ...
    int color_in_array = 0;    // The position in the COLORS_ARRAY[] of the current
                               // color. See next for loop.
    ...
    // color, we need to aply its rotation which is specified in the config.cfg.
    ANTS_ARRAY[ant_i].rotation =
      rotate(ROTATIONS_ARRAY[color_in_array], ANTS_ARRAY[ant_i].rotation);
...
int color2rgb(int color, rgb* rgb_buf) {
    switch (color) {
        case 1:
            rgb_buf->r = 0;
            rgb_buf->g = 0;
